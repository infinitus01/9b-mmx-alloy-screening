...
with tab3:
    st.header("Heuristic-Label Surrogate & Pareto-Style Visualization")
    st.markdown("This module trains on **synthetic labels generated by the 9B-MMX heuristic engine**, not experimental data or CALPHAD results. It is a **prototype demonstration only** for visualization and software testing purposes.")
    st.warning("**Important**: Outputs must not be interpreted as alloy recommendations, material optimization, or validated predictions. All results are based on heuristic teacher labels.")
    ...
    c1, c2 = st.columns(2)
    with c1:
        if st.button("1. Generate Data & Train Heuristic Surrogate", type="primary"):
    ...
    with c2:
        if st.button("2. Perform Pareto-Style Visualization", type="primary"):
    ...
    if 'df_pareto' in st.session_state:
        ...
        st.subheader("Heuristic Trade-off Visualization: Hardness vs. Precipitation Risk")
        ...
        st.subheader("Top 5 Non-Dominated Synthetic Samples")
