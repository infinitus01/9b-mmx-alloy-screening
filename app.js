...
const ELEMENTS = {
  Al: { vec: 3, r: 1.43, T_m: 933, cost: 2.2, name: "鋁 (Al)" },
  Co: { vec: 9, r: 1.25, T_m: 1768, cost: 35.0, name: "鈷 (Co)" },
  Cr: { vec: 6, r: 1.28, T_m: 2180, cost: 9.8, name: "鉻 (Cr)" },
  Fe: { vec: 8, r: 1.26, T_m: 1811, cost: 0.5, name: "鐵 (Fe)" },
  Ni: { vec: 10, r: 1.24, T_m: 1728, cost: 16.5, name: "鎳 (Ni)" }
};
...
let failureLogs = [
  {
    id: "Tainan-VIM-001",
    alloy_name: "Al0.8CoCrFeNi",
    composition: { Al: 20.0, Co: 20.0, Cr: 20.0, Fe: 20.0, Ni: 20.0 },
  ...
  },
  {
    ...
    composition: { Al: 0.0, Co: 20.0, Cr: 35.0, Fe: 22.5, Ni: 22.5 },
  ...
  },
  {
    ...
    composition: { Al: 6.8, Co: 23.3, Cr: 23.3, Fe: 23.3, Ni: 23.3 },
...
// --- 3. Cockpit App State ---
const state = {
  composition: { Al: 18.0, Co: 20.5, Cr: 20.5, Fe: 20.5, Ni: 20.5 },
...
function calculateDescriptors(composition) {
  ...
  return { VEC, delta, dH_mix, omega, T_m, predictedPhase };
}
...
function calculatePenaltyKernel(composition, coolingRate) {
  ...
  failureLogs.forEach(fail => {
    // Composition Euclidean distance squared
    let distanceSq = 0;
...
function predictSurrogateHardness(composition, coolingRate) {
  let baseHardness = 180;
  baseHardness += composition.Al * 12.5; 
  baseHardness += composition.Cr * 4.2;  
  
  if (coolingRate < 1.0) {
    baseHardness -= 30; // annealing relaxation
  }
  
  // Drift representation: based on Al & Cr deviation from core database (which center around 10-15%)
  let devAl = Math.abs(composition.Al - 12);
  let devCr = Math.abs(composition.Cr - 15);
  let drift = 0.5 + 0.05 * (devAl + devCr);
...
function updateDashboard() {
  ...
  descDisplays.omega.innerText = des.omega.toFixed(3);
  ...
  if (des.delta > 6.6) {
    ...
    descDisplays.deltaStatus.innerText = "格點完整一致";
  ...
  if (P_foundry >= 0.4) {
    ...
    predPenaltyRepell.innerText = "HIGH (極高排斥)";
...
function drawFailureRepulsionMap(currentPenalty, currentVEC, currentDelta) {
  ...
  // Dynamic failure coordinate mapping
  // Map Composition Space (Al vs Cr representation as x and y coordinates on canvas)
  // Candidate coordinate
  let candX = (state.composition.Al / 35) * failureKernelCanvas.width;
  let candY = failureKernelCanvas.height - (state.composition.Cr / 50) * failureKernelCanvas.height;
  ...
  failureLogs.forEach(fail => {
    let failX = (fail.composition.Al / 35) * failureKernelCanvas.width;
    let failY = failureKernelCanvas.height - (fail.composition.Cr / 50) * failureKernelCanvas.height;
    ...
    if (fail.defect_type === "LTM_THERMO_DEAD_ZONE") {
      ...
      ctxMap.strokeStyle = "rgba(255, 235, 5 yellow, 0.15)";
      ctxMap.lineWidth = 1;
    }
  ...
  // Draw Repulsion connection line (draws warning vector to the closest defect)
  ...
  if (closestFail) {
    let fX = (closestFail.composition.Al / 35) * failureKernelCanvas.width;
    let fY = failureKernelCanvas.height - (closestFail.composition.Cr / 50) * failureKernelCanvas.height;
...
writeTerminalLine("SYSTEM", "9B-MMX v0.1 材料AI飛控大腦啟動。沙盒隔離環境掛載完畢。", "cyan");
writeTerminalLine("SYSTEM", "讀取 AGENTS.md 完成，防禦性物理合理性護欄 (Physics Consistency Auditor) 配置完成。", "default");
writeTerminalLine("REALITY", "台南實體冶煉廠 (Physical Foundry) 反饋核管道載入成功，共有 3 筆失敗記憶記錄。", "amber");

btnClearLogs.addEventListener('click', () => {
  auditTerminalLogs.innerHTML = "";
});

// --- 11. The 9-Brain Auditing Consensus Simulator ---
btnRunAudit.addEventListener('click', async () => {
  ...
  writeTerminalLine("DIRECTOR", "🎯 審核指令下達：開始對 HEA-Config-#99 進行非同步物理一致性審核與風險防禦評估。", "cyan");
  ...
  // 1. Lattice Architect
  await sleep(800);
  writeTerminalLine("ARCHITECT", `晶格與成分候選結構分析完成。預估晶相結構：[${des.predictedPhase}]。顯式特徵描述符 VEC=${des.VEC.toFixed(3)}, δ=${des.delta.toFixed(3)}%, ΔH_mix=${des.dH_mix.toFixed(2)} kJ/mol.`, "default");
  ...
  writeTerminalLine("AUDITOR", `[1/3] 驗算成分完整性。原子總量和偏離差 = ${Math.abs(totalPct - 100).toExponential(4)} (容差門檻 ${state.systemConfig.tolerance})。結果：✔ 質能守恆通過。`, "green");
  ...
  if (state.process.heat_temp >= T_melt_C) {
    writeTerminalLine("AUDITOR", `🚨 [🚨 物理熔斷 - Level 1 Safety Gate thermal liquefaction]`, "red");
    writeTerminalLine("AUDITOR", `    - 服務溫度 (${state.process.heat_temp}°C) 已超過不銹鋼基體之熱力學熔點 (${T_melt_C.toFixed(1)}°C)！`, "red");
    writeTerminalLine("AUDITOR", `    - 材料狀態：完全熔化為液態鋼水，晶格徹底瓦解，結構強度歸零！`, "red");
    meltingPassed = false;
  } else {
  ...
  if (des.delta > 6.6 || (des.delta > 5.0 && des.VEC < 8.0)) {
    phaseAuditDetails += " [警告] 晶格半徑偏差過大，Laves 脆性析出風險極高！";
    phasePassed = false;
  }
  
  if (!phasePassed) {
    if (state.cmep.enabled) {
      writeTerminalLine("AUDITOR", `[2/3] [CMEP 旁路探索開啟中] 檢測到禁用脆性相析出威脅：${phaseAuditDetails}。因人工授權 CMEP 非平衡旁路，特許描述符數值漂移探索，免予自動熔斷！`, "amber");
    } else {
      writeTerminalLine("AUDITOR", `[2/3] [⚠️ 禁用相熔斷警告] 檢測到析出相一致性異常：${phaseAuditDetails}。高度脆性開裂預警！`, "red");
    }
  } else {
  ...
  await sleep(950);
  writeTerminalLine("AUDITOR", `[3/3] 實體反饋核 (Foundry Feedback Kernel) 計算完成。距離台南失敗known failure region最近點為 [${pen.penaltyDetails[0].alloy_name}]: 距離為 ${pen.penaltyDetails[0].distance.toFixed(2)} at.%, 實體排斥力總分 P_foundry = ${pen.P_foundry.toFixed(4)}.`, pen.P_foundry >= 0.4 ? "red" : (pen.P_foundry >= 0.25 ? "amber" : "green"));

  // 4. Safety Guardian
  await sleep(700);
  if (state.composition.Al > 15 && state.process.heat_temp > 1000) {
    writeTerminalLine("GUARDIAN", "高 Al 含量在 1000°C 以上真空度不足時極易產生高溫揮發與氧化渣，安全評估為 [中等風險]。", "amber");
  } else {
  ...
  await sleep(600);
  writeTerminalLine("INTEGRATOR", "傳輸阻尼評估完成。電子聲子耦合描述符一致，無數值消散或信息失真幻覺，特徵流完整。", "default");
  ...
  await sleep(900);
  let reportSafe = pen.P_foundry < 0.25 && phasePassed && meltingPassed;
  let riskNav = "";
  
  if (!meltingPassed) {
    riskNav = `【🚨 物理熔斷：1600°C thermal liquefaction警報】服務溫度 ${state.process.heat_temp}°C ＞ 材料熔點 ${T_melt_C.toFixed(1)}°C！在該高溫與 ${state.process.stress} MPa 應力下，鐵基不銹鋼已液化為鋼水，晶格徹底瓦解！\n` + 
              `💡 飛控大腦修正指南：\n` +
              `  1. 難熔高熵合金 (RHEA) 路線：放棄 Fe 基，改用 W-Mo-Ta-Nb 難熔體系。Tm > 2500°C，在 1600°C 下 Th ≈ 0.67，具備極強的高溫抗蠕變能力。\n` + 
              `  2. 陶瓷基複合材料 (UHTCMC) 路線：如 C/SiC (碳纖維碳化矽)，耐溫超 1800°C。\n` + 
              `  3. 熱障塗層與主動冷卻：設計 YSZ 隔熱層，使金屬工作溫度保持在 1100°C 以下。`;
    writeTerminalLine("TRANSLATOR", riskNav, "red");
    footerReportSummary.innerText = "【Level 1 Safety Gate 熔毀阻斷】: 1600°C 高溫高壓大幅超出不銹鋼熔點！結構已完全液化塌陷，強制中止！";
    ...
    riskNav = `【CMEP 非平衡探索 bypass 狀態】當前成分屬於亞穩態邊緣。VEC=${des.VEC.toFixed(3)}, P_foundry=${pen.P_foundry.toFixed(4)}。特許將隔離沙盒 ` + 
    ...
    footerReportSummary.innerText = "【CMEP 亞穩態探索報告】: 允許物理描述符漂移，隔離沙盒掛載中。實體偏析風險高，laboratory validation planning建議快冷。";
    footerReportSummary.parentElement.querySelector('.ticker-badge').className = "ticker-badge ticker-warning";
    footerReportSummary.parentElement.querySelector('.ticker-badge').innerText = "亞穩探索";
  } else if (reportSafe) {
    riskNav = `【通過審查】此候選配方通過物理合理性與實體反饋核排斥力粗篩！P_foundry = ${pen.P_foundry.toFixed(4)}，無硬性相熔斷。` + 
              `此候選配方僅通過描述符篩選與 surrogate hardness estimate 代理模型預估，仍屬未驗證狀態。在進行任何工程應用前，實體物理測試與金相表徵為強制性流程。`;
    writeTerminalLine("TRANSLATOR", riskNav, "green");
    footerReportSummary.innerText = "【9B-MMX 審核通過】: 候選配方描述符過濾通過，與台南失敗known failure region距離足夠，可進行 TRL-1 階段laboratory validation planning。";
    ...
    if (pen.P_foundry >= 0.4) {
      failWarn = `與 Tainan-VIM-001 (Al0.8CoCrFeNi 熱力學死路) 空間極度接近！若在當前冷卻速率下實體熔煉，極易產生脆性 B2/σ 相析出開裂。`;
    } else {
      failWarn = `檢測到高風險 Laves/σ 相脆性析出可能。實體反饋核總懲罰值 P_foundry = ${pen.P_foundry.toFixed(4)} 處於排斥狀態。`;
    }
    riskNav = `【高度風險警告】${failWarn} 系統已將該成分列為物理排斥區！強烈建議對成分進行調參或更改工藝氣氛冷卻速率以避開known failure region。` + 
              `※ 聲明：冶金研發首要為風險導航，不可使用完美虛擬數值代替重力與裂紋！※`;
    writeTerminalLine("TRANSLATOR", riskNav, "red");
    footerReportSummary.innerText = "【高度風險阻斷】: 與台南失敗known failure region空間極度接近！B2/σ 脆性相開裂風險極高，強烈建議更改配方或冷卻速率。";
    footerReportSummary.parentElement.querySelector('.ticker-badge').className = "ticker-badge ticker-warning";
    footerReportSummary.parentElement.querySelector('.ticker-badge').innerText = "高度警告";
  }

  writeTerminalLine("SYSTEM", "9B-MMX v0.1 非同步審計流程完工。物理報告已寫入 logs/physics_audit_report.json (虛擬唯讀沙盒掛載)。", "cyan");
...
btnCmepToggle.addEventListener('click', () => {
  ...
  if (state.cmep.enabled) {
    ...
    writeTerminalLine("DIRECTOR", "⚠️ [CMEP 旁路解鎖] 人工總監授權 CMEP 協議。解鎖高維搜尋空間描述符漂移，封鎖 Validated DB 自動寫入，隔離 sandbox worktree 鎖定。", "red");
    ...
    writeTerminalLine("DIRECTOR", "🔒 [CMEP 旁路關閉] 人工總監關閉 CMEP 協議。重新啟用全量描述符物理合理性熔斷，恢復 Level 1 Safety Gate 最高限制。", "cyan");
    writeTerminalLine("SYSTEM", "全域安全等級恢復為: Level 1 Safety Gate. 強制守恆與脆性析出相一致性門檻恢復。", "default");
  }
  updateDashboard();
});
...
btnSubmitFailure.addEventListener('click', () => {
  ...
  if (!alloy || isNaN(f_Al) || isNaN(f_Cr) || isNaN(f_cool) || !details) {
    alert("請確實填寫所有台南失敗開爐紀錄數據！");
    return;
  }

  // Define failure coordinate representation
  const newFail = {
    id: `Tainan-Manual-${Date.now().toString().slice(-4)}`,
    alloy_name: alloy,
    composition: {
      Al: f_Al,
      Co: 20.0, // default placeholder elements normalized
      Cr: f_Cr,
      Fe: 20.0,
      Ni: 20.0
    },
  ...
  // Log in terminal
  writeTerminalLine("REALITY", `⚠️ 台南實體冶煉回灌 [${alloy}] 失敗記憶成功！缺陷類型: ${type}，成分座標 Al=${f_Al} at.%, Cr=${f_Cr} at.%，金相: ${details}。已生成新的多維懲罰排斥核。`, "amber");
...
setInterval(() => {
  ...
  if (state.cmep.enabled) {
    ...
    if (state.cmep.window <= 0) {
      ...
      writeTerminalLine("SYSTEM", "🚨 [CMEP 探索窗口過期自毀] 已達到 500 次最大迭代上限。CMEP 進程自動熔斷自毀，已銷毀所有沙盒中間體，返回安全狀態！", "red");
      updateDashboard();
    }
  }
}, 100);
...
btnSaveConfig.addEventListener('click', () => {
  ...
  writeTerminalLine("SYSTEM", "⚙ AGENTS.md 隔離配置重載成功。重新編譯並加載飛控安全閘門...", "cyan");
