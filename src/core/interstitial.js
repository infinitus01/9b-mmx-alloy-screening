...
/**
 * Calculates the experimental nitrogen solubility limit using Sieverts' Law at 1600°C, 1 atm N2 gas.
 * This is an optional experimental descriptor and does NOT replace the baseline Phase 2 linear gate.
 */
function calculateExperimentalSievertsNLimit(composition) {
  ...
  // First-order interaction coefficients at 1873 K
  const e_N_Cr = -0.06;
  const e_N_Mn = -0.02;
  const e_N_Ni = +0.01;
  const e_N_Co = +0.01;
  ...
  const c_N_base_wt = 0.045; // wt.% solubility in pure liquid iron
  const c_N_max_wt = c_N_base_wt * Math.pow(10, -log_f_N);

  // Convert wt.% back to at.% for direct comparison with sliders
  let totalMass = 0;
  let totalAt = 0;
